    ...
    @if ($prod->user->is_vendor == 2)
        @if (isset($_GET['max']))
            @if ($prod->vendorPrice() <= $_GET['max'])
                <div class="w-100 h-100 remove-padding">
                    <a href="{{ route('front.product', $prod->slug) }}" class="item d-flex flex-column justify-content-between">
                        @if (!is_null($prod->discount_percent))
                            <span class="badge badge-danger descont-card text-center py-3">
                        ...
                        @endif
                        <div
                            class="item-img position-static d-flex flex-column justify-content-center align-items-center w-60 mx-auto mb-2 {{ $gs->show_products_without_stock_baw && !is_null($prod->stock) && $prod->stock == 0 ? 'baw' : '' }}">
                            @if ($admstore->reference_code == 1)
                                @php $prod = App\Models\Product::findOrFail($prod->id); @endphp
                                <div class="sell-area ref offer-ref">
                        ...
                        <div class="deal-counter pl-4 py-0">
                            <h6>Acaba em</h6>
                            <div data-countdown="{{ $prod->discount_date }}"></div>
                        </div>
                        @if ($gs->is_cart)
                            <div class="item-cart-area w-100 px-2 mt-2 mb-3 d-flex justify-content-center mb-3">
        ...
        @else
            <div class="col-lg-3 col-md-3 col-6 remove-padding">
                ...
                <a href="{{ route('front.product', $prod->slug) }}" class="item">
                    ...
                    <div
                        class="item-img d-flex flex-column justify-content-center align-items-center w-60 mx-auto mb-0 {{ $gs->show_products_without_stock_baw && !is_null($prod->stock) && $prod->stock == 0
                            ? "
                                                                baw"
                            : '' }}">
                        @if ($admstore->reference_code == 1)
...
@else
    <div class="w-100 h-100 remove-padding">
        <a href="{{ route('front.product', $prod->slug) }}" class="item d-flex flex-column justify-content-between">
            @if (!is_null($prod->discount_percent))
                <span class="badge badge-danger descont-card text-center py-3">
                    <span class='d-block pl-2' style="font-size: 26px;">
                        {{ $prod->discount_percent . '%' }} &nbsp;
                    </span> 
                    <span class='d-block'>
                        {{ 'OFF' }}
            ...
            @endif
            <div
                class="item-img position-static d-flex flex-column justify-content-center align-items-center w-60 m-auto mb-2 {{ $gs->show_products_without_stock_baw && !is_null($prod->stock) && $prod->stock == 0 ? " baw" : '' }}">
                @if ($admstore->reference_code == 1)
                    @php $prod = App\Models\Product::findOrFail($prod->id); @endphp
                    <div class="sell-area ref offer-ref">
                        <span class="sale">{{ $prod->ref_code }} </span>
                    ...
                    @else
                        ...
                        @else
                            @if ($gs->is_cart_and_buy_available)
                                ...
                                {{-- <span class="add-to-cart-quick shadow-sm add-to-cart-btn d-block" data-href="{{ route('product.cart.quickadd', $prod->id) }}">
                                    {{ __('Buy Now') }}
                                    <i class="fas fa-plus-circle"></i>
                                </span> --}}
                            @else
