@if (count($prods) > 0)
    @foreach ($prods as $key => $prod)
        @if ($prod->user_id == 0)
            <div class="feat_card mb-3 mx-lg-0 mx-xl-xl-0 remove-padding">
                <div class="remove-padding w-100 h-100">
                    <a href="{{ route('front.product', $prod->slug) }}" class="item d-flex flex-column">
                        ...
                        @endif --}}
                        <div
                            class="item-img d-flex flex-column justify-content-center align-items-center w-100 mx-auto mb-0 position-static {{ $gs->show_products_without_stock_baw && !is_null($prod->stock) && $prod->stock == 0
                                ? "
                                        baw"
                                : '' }}">
                            ...
                            <div class="extra-list w-100">
                                <ul class='d-flex justify-content-center align-items-center'>
                                    ...
                                    <li>
                                        <span class="quick-view" class='shadow-sm' rel-toggle="tooltip" title="{{ __('Quick View') }}"
                                    ...
                                    <li>
                                        <span class="add-to-compare" class='shadow-sm' data-href="{{ route('product.compare.add', $prod->id) }}"
                        ...
                        <div class="info px-4">
                            ...
                            @if (!config('features.marketplace'))
                                <h5 class="name px-1">{{ $prod->capitalize_name }}</h5>
                            ...
                            @else
                                <h4 class="price px-1">{{ $prod->showVendorMinPrice() }} até {{ $prod->showVendorMaxPrice() }}
                        ...
                        @if ($gs->is_cart)
                            <div class="item-cart-area w-100 mt-2 px-2 px-lg-3 d-flex justify-content-center">
                                @if ($prod->product_type == 'affiliate')
                                    <span class="add-to-cart-btn affilate-btn shadow-sm"
                                ...
                                @else
                                    ...
                                    @else
                                        @if ($gs->is_cart_and_buy_available)
                                            <span class="add-to-cart add-to-cart-btn shadow-sm mr-2"
                                            ...
                                            <span class="add-to-cart-quick add-to-cart-btn shadow-sm"
                                                data-href="{{ route('product.cart.quickadd', $prod->id) }}">
                                                {{ __('Buy Now') }}
                                                <i class="fas fa-plus-circle"></i>
                                            </span>
                                        @else
                                            <span class="add-to-cart-btn shadow-sm"
