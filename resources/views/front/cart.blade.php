...
@section('content')
    ...
    <div class="breadcrumb-area m-0 bg-light">
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
                    <ul class="pages">
    ...
    <!-- Cart Area Start -->
    <section class="cartpage" style="background-color: #fff;">
        <div class="container">
            <div class="row">
                <div class="col-lg-8">
                            ...
                            <table class="table">
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th scope='col' width="30%">{{ __('Details') }}</th>
                                        <th scope='col'>{{ __('Unit Price') }}</th>
                                        <th scope='col'>{{ __('Sub Total') }}</th>
                                        <th scope='col'><i class="icofont-close-squared-alt"></i></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @if (Session::has('cart'))
                                    ...
                                    ,'),
                                            ...
                                            $custom_item_id = str_replace(['\'', '"', ',', '.', ' ', ';', '<', '>'], '', $custom_item_id); @endphp <tr class="cremove{{ str_replace(['~', '/', '-'],'',$custom_item_id) }}">
                                                <td class="product-img" scope='row'>
                                                    <div class="item">
                                                        @if(basename($product["item"]->thumbnail) !== 'noimage.png' && $product["item"]->thumbnail && !filter_var($product["item"]->thumbnail, FILTER_VALIDATE_URL))
                                                            ...
                                                            : asset('storage/images/thumbnails/' . $product["item"]->thumbnail) }}" alt="">
                                                        ...
                                                        @elseif(basename($product["item"]->thumbnail) !== 'noimage.png' && $product["item"]->thumbnail && filter_var($product["item"]->thumbnail, FILTER_VALIDATE_URL))
                                                            <img class="img-fluid" src="{{ $product["item"]->thumbnail }}" alt="">

                                                        @elseif(basename($product["item"]->photo) !== 'noimage.png' && $product["item"]->photo && !filter_var($product["item"]->photo, FILTER_VALIDATE_URL))
                                                            <img class="img-fluid" src="{{ filter_var($product["item"]->photo, FILTER_VALIDATE_URL) ? $product["item"]->photo
                                                                : asset('storage/images/products/' . $product["item"]->photo) }}" alt="">
                                                        @elseif(basename($product["item"]->photo) !== 'noimage.png' && $product["item"]->photo && filter_var($product["item"]->photo, FILTER_VALIDATE_URL))
                                                            <img class="img-fluid" src="{{ $product["item"]->photo }}" alt="">
                                                        @elseif(isset($product["item"]->galleries[0]) && filter_var($product["item"]->galleries[0]->photo, FILTER_VALIDATE_URL))
                                                            <img class="img-fluid" src="{{ $product["item"]->galleries[0]->photo }}" alt="">
                                                        @elseif(isset($product["item"]->galleries[0]))
                                                            <img class="img-fluid" src="{{ asset('storage/images/galleries/'.$product["item"]->galleries[0]->photo) }}" xoriginal="{{ $product["item"]->galleries[0]->photo }}" />
                                                        @else
                                                ...
                                                <td class="unit-price quantity">
                                                    ...
                                                    <p class="product-unit-price text-secondary" style="font-size: small;">
                    ...
                    <div class="col-lg-4">
                        <div class="right-area">
                            <div class="order-box">
                                ...
                                @if ($gs->is_standard_checkout)
                                    <a href="{{ route('front.checkout') }}" class="order-btn shadow-sm bg-light text-dark">
                                        {{ __('Place Order') }}
                                    </a>
                                @endif

                                @if ($gs->is_simplified_checkout && !empty($gs->simplified_checkout_number))
                                    <a href="#" class="order-btn mt-2 shadow-sm bg-light text-dark" data-toggle="modal"
