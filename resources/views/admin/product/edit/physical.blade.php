                                    ...
                                    <div class="col-xl-6">
                                        <div class="input-form">
                                            <div class="d-flex justify-content-between">
                                                <h4 class="heading">
                                                    {{ __('Adicione link de imgens') }}
                                                    <span>
                                                        {{ __('(Optional)') }}
                                                    </span>
                                                </h4>
                                                <i id="add-new-link" style="font-size: 20px;line-height: 0px;font-style: normal;padding: 8px; cursor:pointer;">+</i>
                                            </div>
                                            <input name="img_links[]" type="text" class="input-field img-input-link"
                                                placeholder="{{ __('Enter Photo URL') }}"
                                                value="">
                                        </div>
                                        <div class="input-form"
                                    ...
                                    @if(env('ENABLE_OPENAI_API'))
                                        <div class="col-12">
                                            <div class="input-form">
                                                ...
                                                <input name="gpt_details" type="text" class="input-field img-input-link"
                                                    placeholder="MACBOOK AIR 2020 M1 256GB GRAY"
                                                    value="{{$data->gpt_details}}">
                                                <div class="col-xl-6 text-right">
                                                    <button class="addProductSubmit-btn" type="button" onclick="askGpt()"><img class="mr-2 mb-1" height="20" width="20" src="{{asset('assets/images/icon/logochat.png')}}" alt="">{{ __('Complete with GPT') }}</button>
                                                </div>
                                            </div>
                                        </div>
                                    @endif
...
@section('scripts')
    ...
    <script>
        var envEnable = {{env('ENABLE_OPENAI_API')}}
        $(document).ready(function() {
            if(envEnable){
                var editor = document.getElementsByClassName("trumbowyg-editor")[0];
                if(editor){
                    var pTag = editor.querySelector("p");
                    if(!pTag){
                        var newPTag = document.createElement("p");
                        newPTag.innerHTML = ' '
                        editor.appendChild(newPTag);
                    }
                }
            }
        ...
        if(envEnable){
            function askGpt(){
                document.querySelector('.trumbowyg-editor').innerHTML = '<p></p>'
                var inputValue = document.getElementsByName('gpt_details')[0].value
                ...
                fetch('/admin/products/api/openai', {
                    method: 'POST',
                    headers: {
                    ...
                    .then(data => {
                        document.getElementsByClassName("trumbowyg-editor")[0].querySelector("p").innerHTML = data.choices[0].message.content;
                        document.getElementById("textarea-desc").value = data.choices[0].message.content;
                    })
                    .catch((error) => {
    ...
    <script>
        function isLink(value) {
            // Expressão regular para verificar se a string parece ser um link
            var urlRegex = /^(ftp|http|https):\/\/[^ "]+$/;
            // Testar a variável contra a expressão regular
    ...
    <script>
        $(document).ready(function() {
            let html =
                `<img src="{{ (basename($data->photo) !== 'noimage.png' && $data->photo) ? asset('storage/images/products/' . $data->photo) : ((isset($data->galleries[0]) && filter_var($data->galleries[0]->photo, FILTER_VALIDATE_URL)) ? $data->galleries[0]->photo : (isset($data->galleries[0]) ? asset('storage/images/galleries/' . $data->galleries[0]->photo) : asset('storage/assets/images/noimage.png'))) }}" alt="">`;
    ...
    <script>
        ...
        var btnPlus = document.getElementById('add-new-link')
        var inputLink = document.querySelector('.img-input-link')
        btnPlus.addEventListener('click', () => {
            var divClone = inputLink.cloneNode(true);
            btnPlus.parentNode.parentNode.appendChild(divClone);
        })
    </script>
