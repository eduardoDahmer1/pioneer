...
use App\Jobs\ProcessOpenAiJson;
...
class ProductController extends Controller
{
    ...
    {
        $products = Product::search($request->q)->take(100)->get();
        return view('scout', compact('products'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if (session('product_json')) {
            $data['description_gpt'] = session('product_json');
            session()->forget('product_json');
        }
        ...
        if (env('ENABLE_OPENAI_API')) {
            $data['gpt_details'] = $input['gpt_details'];
            $message = $request->input('gpt_details');
            ProcessOpenAiJson::dispatch($message, $data->id);
        }
        $linkPhotos = $request['img_links'];
    ...
    public function edit($id)
    {
        ...
        $gpt_details = !empty($data->gpt_details);
